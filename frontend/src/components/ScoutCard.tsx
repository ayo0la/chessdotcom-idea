...
export default function ScoutCard({ username }: ScoutCardProps) {
  ...
  return (
    <section className="card mt-6 p-4 sm:p-5 animate-rise">
      <p className="kicker">Know your enemy</p>
      <h2 className="mb-1 font-display text-lg font-bold">Scouting Report</h2>
      <p className="text-sm text-gray-300 mb-4">
        ...
        {streakText && (
          <span
            className={
              recentForm.streak > 0 ? "text-emerald-400" : "text-amber-400"
            }
          >
            {" "}
            · {streakText}
        ...
        <div className="mb-3">
          ...
          <ul className="space-y-1">
            {weapons.map((o) => (
              <li key={o.eco} className="text-sm flex justify-between gap-2">
                <span className="text-gray-200 truncate">{o.name}</span>
                <span className="whitespace-nowrap tabular-nums text-emerald-400">
                  {o.winRate}% · {o.games} games
                </span>
              </li>
            ))}
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {[style.labels.style, style.labels.approach, style.labels.clock].map(
            (label) => (
              <span
                key={label}
                className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-0.5 text-xs font-medium text-gray-300"
              >
                {label}
              </span>
            )
