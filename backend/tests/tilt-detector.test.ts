...
import {
  ...
  analyzeTilt,
  checkTiltForUser,
} from "../src/services/analysis/tilt-detector";

const NOW = new Date("2026-07-02T12:00:00Z");

function pgnWith(timeControl: string, whiteClks: string[], blackClks: string[]): string {
  const moves: string[] = [];
  const n = Math.max(whiteClks.length, blackClks.length);
  for (let i = 0; i < n; i++) {
    let move = `${i + 1}.`;
    if (whiteClks[i]) move += ` e4 {[%clk ${whiteClks[i]}]}`;
    if (blackClks[i]) move += ` e5 {[%clk ${blackClks[i]}]}`;
    moves.push(move);
  }
...
describe("checkTiltForUser", () => {
  ...
  it("creates a TiltEvent when the user is tilting", async () => {
    ...
    expect(db.tiltEvent.create).toHaveBeenCalledWith(
  ...
  it("does not create a duplicate when a recent TiltEvent exists", async () => {
    ...
    vi.mocked(db.tiltEvent.findFirst).mockResolvedValue({ id: "existing" } as any);
