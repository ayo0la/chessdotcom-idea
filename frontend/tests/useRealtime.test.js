...
describe("useRealtime", () => {
    it("subscribes to Rating changes on mount for the active tab", () => {
        ...
        expect(mockChannel.on).toHaveBeenCalledWith("postgres_changes", expect.objectContaining({ event: "UPDATE", table: "Rating", filter: "timeControl=eq.blitz" }), expect.any(Function));
    ...
    it("calls onUpdate with username, delta, and new rating when a matching userId changes", () => {
        ...
        const pgChangeCallback = vi.mocked(mockChannel.on).mock.calls[0][2];
        pgChangeCallback({
